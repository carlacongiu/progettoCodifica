<?xml version="1.0" encoding="UTF-8" ?> 
                ...
                <xsl:element name="div">
                    ...
                    <xsl:element name="ul">
                        ...
                        <xsl:element name="li">
                            ...
                            <xsl:element name="a">
                                <xsl:attribute name="href">#cartolina_C1</xsl:attribute>
                                Cartolina n.40
                            </xsl:element>
                        </xsl:element>
                        <xsl:element name="li">
                            ...
                            <xsl:element name="a">
                                <xsl:attribute name="href">#cartolina_C2</xsl:attribute>
                                Cartolina n.50
                            </xsl:element>
                        </xsl:element>
                        <xsl:element name="li">
                            ...
                            <xsl:element name="a">
                                <xsl:attribute name="href">#cartolina_C3</xsl:attribute>
                                Cartolina n.223
                            </xsl:element>
                        </xsl:element>
                        <xsl:element name="li">
                                ...
                                <xsl:when test="$car='C1' or $car='C3'">
                                    <xsl:element name="div">
                                        ...
                                        <xsl:element name="div">
                                            ...
                                            <h3>Persone</h3>
                                            <xsl:apply-templates select="tei:teiHeader//tei:listPerson//tei:person"/>
                                        </xsl:element>
                                        <xsl:element name="div">
                                            <xsl:attribute name="class">info</xsl:attribute>
                                            <h3>Luoghi</h3>
                                            <xsl:apply-templates select="tei:teiHeader//tei:listPlace//tei:place"/>
                                        </xsl:element>
                                        <xsl:element name="div">
                                            <xsl:attribute name="class">info</xsl:attribute>
    ...
    <xsl:template match="tei:supportDesc">
        <xsl:element name="p">
            <b>Stato di conservazione della cartolina: </b><xsl:value-of select="tei:condition"/>
        </xsl:element>        
    </xsl:template>
    ...
    <xsl:template match="tei:person">
        <xsl:element name="p">     
           <xsl:apply-templates select="tei:persName"/>- <xsl:apply-templates select="tei:sex"/><br/>
        </xsl:element>
    </xsl:template>

    <xsl:template match="tei:place">
        <xsl:element name="p">    
           <xsl:apply-templates select="tei:placeName"/> (<xsl:apply-templates select="tei:region"/> - <xsl:apply-templates select="tei:country"/>)<br/>
        </xsl:element>
    </xsl:template>
    ...
    <xsl:template match="tei:text//tei:div[@type='retro']">
        ...
        <xsl:element name="p">
            ...
            <xsl:apply-templates select="tei:div[@type='message']/tei:p//tei:expan"/>
        ...
        <xsl:element name="p">
            ...
            <xsl:apply-templates select="tei:div[@type='destination']//tei:address//tei:expan"/>
            <xsl:value-of select="tei:div[@type='destination']//tei:address"/>
        </xsl:element>
    </xsl:template>

    <xsl:template match="tei:div[@type='message']/tei:p//tei:expan">
        <span class="expan">
            <xsl:value-of select="."/>
        </span>
    </xsl:template>

    <xsl:template match="tei:address//tei:expan">
        <span class="expan">
            <xsl:value-of select="."/>
        </span>
    </xsl:template>
